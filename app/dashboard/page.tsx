...
import { MySidebar } from "@/components/main/my-sidebar";
...
const ErrorDisplay = ({ message }: { message: string }) => (
    ...
    >
        <svg viewBox="0 0 100 100" className="w-28 h-28" fill="none" stroke="black" strokeWidth="8" strokeLinecap="round">
            <circle cx="50" cy="50" r="44" strokeWidth="6" opacity="0.12" fill="black" fillOpacity="0.04" />
...
const CancelledDisplay = () => (
    ...
    >
        <svg viewBox="0 0 100 100" className="w-20 h-20" fill="none" stroke="black" strokeWidth="6" strokeLinecap="round">
            <circle cx="50" cy="50" r="44" strokeWidth="6" opacity="0.15" fill="black" fillOpacity="0.04" />
            <line x1="35" y1="50" x2="65" y2="50" />
        </svg>
        <p className="text-sm font-black text-black/60 uppercase tracking-widest">Generation cancelled</p>
    </motion.div>
);
...
const MemeDisplay = ({ dataUrl }: { dataUrl: string }) => {
    ...
    return (
        <motion.div
            ...
            exit={{ opacity: 0 }} transition={{ type: "spring", stiffness: 200, damping: 22 }}
        ...
        >
            <div className="relative border-4 border-black shadow-[8px_8px_0_0_rgba(0,0,0,1)] rounded-xl overflow-hidden bg-white max-w-sm w-full">
                ...
                <img src={dataUrl} alt="Generated zzal" className="w-full h-auto" />
                <button
                    onClick={handleDownload}
                    className="absolute bottom-3 right-3 flex items-center gap-1.5 bg-black text-white text-xs font-black uppercase tracking-widest px-3 py-2 rounded-lg hover:translate-y-[-1px] transition-all"
                >
            ...
            <p className="text-xs font-bold text-black/40 uppercase tracking-widest animate-pulse">✨ Zzal ready!</p>
        </motion.div>
    );
};
...
export default function DashboardPage() {
    ...
    // Shared delete state — syncs deletion across gallery and sidebar
    const [deletedIds, setDeletedIds] = React.useState<Set<string>>(new Set());

    const handleDeleteSync = React.useCallback((id: string) => {
                    ...
                    >
                        <AnimatePresence mode="wait">
                            {(genStatus === "generating" || genStatus === "uploading") && (
                                <motion.div key="loading" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
                                    <LoadingBottle />
                                </motion.div>
                            )}
                            {genStatus === "error" && (
